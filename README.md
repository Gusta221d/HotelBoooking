...
Estudo experimental reprodutível de clustering sobre reservas hoteleiras (Unsupervised Learning 2025/2026).
...
1. Obter `hotel_bookings_course_release_v1.csv` (course release v1 na plataforma da UC ou [Kaggle](https://www.kaggle.com/datasets/jessemostipak/hotel-booking-demand)).
...
- **Tempo típico:** ~15–20 minutos (dataset completo, inclui iK-Means).
- **Entrada única:** `runAll.py` apaga outputs anteriores e corre por ordem: `edaClean` → `edaVisuals` → `train_models` → `estabel_hierar` → `cluster_profile` → `e4_pca_svd`.
- **Rastreio:** `src/pipeline_run.json` (`run_id`) e `src/experiments.csv`.
...
A submissão final está identificada pela tag/release **`Submission`** no GitHub.
